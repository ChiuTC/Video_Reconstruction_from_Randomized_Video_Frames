...
using namespace std;
//===============================================定義函式與變數
float BlackBox(int row, int col, cv::Mat& U, cv::Mat& img_orig, float& d);
...
void CF::Solver(const int Type, double& time, const int MaxItNum, const float lambda, const float DataFitOrder, const float stepsize)
{
    ...
    ofstream energyProfile("Energy.txt");
...
#pragma omp parallel for
            for (int j = 1; j < N - 1; ++j)
            {
                float (*BlackBox_co)(int row, int col, Mat & U, Mat & img_orig, float& d) = BlackBox;
                dataFit.at<float>(i, j) = BlackBox_co(i, j, imgF, image, zero);
            }
        }
        ...
        {
#pragma omp parallel for
            for (int i = start_row_set[set_index]; i < M - 1; i+=2)
            {
                float* p = imgF.ptr<float>(i);
                float* p_pre = imgF.ptr<float>(i - 1);
                float* p_down = imgF.ptr<float>(i + 1);
#pragma omp parallel for
                for (int j = start_col_set[set_index]; j < N - 1; j+=2)
                {
                    
                    float (*BlackBox_co)(int row, int col, Mat & U, Mat & img_orig, float& d) = BlackBox;
                    float d = stepsize * (this->*Local)(j, p_pre, p, p_down, NULL);
                    float dist_orig = BlackBox_co(i, j, imgF, image, zero);
                    float dist_proj_orig = BlackBox_co(i, j, imgF, image, d);
                    float  energy_increase = dist_proj_orig - dist_orig;
                    if (energy_increase <= lambda * abs(d)) p[j] += d;
                }
            }
        }
